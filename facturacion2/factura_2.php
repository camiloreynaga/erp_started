...
$consultaVenta = mysql_query("SELECT id_cliente, fecha_venta,forma_pago FROM venta WHERE id ='$idVenta' ",$link);
$venta= mysql_fetch_array($consultaVenta);


//-----------------consulta tabla cliente------------------------------//
$consultaCliente = mysql_query("SELECT id, nombre, ruc, direccion FROM cliente WHERE id ='$venta[0]' ",$link);
...
$consultaDetallle ="SELECT id_venta, id_producto,lote, cantidad, precio_unitario,fecha_vencimiento FROM  detalle_venta WHERE id_venta='$idVenta'"; 
...
while($row2=  mysql_fetch_array($detalle))
		...
		array_push($precio,$row2[4]);
                ...
                $consultaFechaVenc= "SELECT fecha_vencimiento FROM detalle_compra WHERE producto='$row2[1]' and lote='$row2[2]'";  
...
for($j=0;$j<$lineas;$j++)
{
    ...
    $consultaProducto= mysql_query("SELECT nombre_producto FROM producto WHERE id='$id_producto[$j]'",$link);
...
// fecha 25 dÃ­as despues para el pago del la factura

if(strtolower($venta[2])=="cash" | strtolower($venta[2])=="contado")
{
    $fecha1=$fecha;
...
{
    if(strtolower($venta[2])=="credit" |strtolower($venta[2])=="credito"  )
         $condiciones_pago="CREDITO";
    if(strtolower($venta[2])=="letter" | strtolower($venta[2])=="letra" )
...
$dataTotales = array( array('dato'=>"<b>      </b> ".strtoupper(numerotexto($total)), 'dato2'=>"       ", 'dato3'=>number_format($subTotal,2),'dist'=>""),array('dato'=>"", 'dato2'=>" ",'dato3'=>"",'dist'=>""),array('dato'=>"", 'dato2'=>" ",'dato3'=>number_format($igv,2),'dist'=>""),array('dato'=>"", 'dato2'=>" ",'dato3'=>"",'dist'=>""),array('dato'=>"", 'dato2'=>"<b>   </b>",'dato3'=>number_format($total,2),'dist'=>""));
///------------------------------------------------
