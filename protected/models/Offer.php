...
class Offer extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('offer, discount_rate, start_date', 'required'),
			array('active, create_user_id, update_user_id', 'numerical', 'integerOnly'=>true),
			array('offer', 'length', 'max'=>50),
			array('discount_rate', 'length', 'max'=>10),
			array('condition, expiration_date, create_time, update_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, offer, discount_rate, condition, start_date, expiration_date, active, create_time, create_user_id, update_time, update_user_id', 'safe', 'on'=>'search'),
		);
	}
