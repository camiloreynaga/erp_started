 ...
 */
class Empresa extends Erp_startedActiveRecord//CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('razon_social, ruc, direccion', 'required'),
			array('activo, create_user_id, update_user_id', 'numerical', 'integerOnly'=>true),
			array('razon_social, direccion', 'length', 'max'=>50),
			array('ruc', 'length', 'max'=>20),
			array('observacion, create_time, update_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, razon_social, ruc, direccion, observacion, activo, create_time, create_user_id, update_time, update_user_id', 'safe', 'on'=>'search'),
		);
	}
